...
| STR-05 | `what_to_do_next` has a **50 ms** wall-clock budget; exceeding it (or raising) forfeits that turn — it never crashes the match. The forfeit is recorded in the replay (`strategy_error` / `strategy_timeout` events) and narrated by the match window's Events ticker, not just printed to the console. | ✅ |
...
| VIS-04 | Controls: play/pause (Space), step ±1 (←/→, hold to repeat), Home/End, speed 0.25×–16×, and a **jump-to-turn slider**. Wheel zoom is anchored at the cursor; pan via left-drag (click-without-drag selects a bot) or middle-drag, clamped so the map can't be lost; F (or loading any replay) fits the whole map in view; **C follows the selected bot** (manual pan releases). Events-ticker rows are clickable to jump to that turn. A hint line in the top bar lists the shortcuts. | ✅ |
| VIS-05 | HUD: map name, turn counter, per-player score + bots alive (labels are 1-indexed: "Player 1"/"Player 2"), winner line, map legend, and an always-visible bot inspector. The inspector shows a one-sentence plain-language description of the selected bot's type (single-sourced from `data/bot_types.json`) above its stats. | ✅ |
...
| ID | Requirement | Status |
|---|---|---|
| TRN-01 | Round-robin over a **user-chosen field of competitors** (added via a file browser, one at a time or several per visit with multi-select; removable per row; long fields scroll) on a **user-chosen set of maps** ("Maps..." multi-select; default: every shipped map). *Revision 4 (v0.0.17): replaces "everything in strategies/"; maps became selectable in v0.0.20.* | ✅ |
...
| UX-01 | **Replay browser**: the match window's "Replays..." button lists **every** saved replay (tournament and headless runs included), newest first with age labels, scrollable, with a per-row [x] to delete old replays. A replay whose map file has since been deleted shows an explicit banner instead of silently rendering on blank terrain. | ✅ |
